...
Then use Kotlin DSL signing in `android/app/build.gradle.kts` (already configured in this repo):
...
### Verify You Are Using The Correct Play Upload Key

Google Play expects your upload certificate SHA1 to match exactly.

Check your keystore fingerprint:

```bash
keytool -list -v \
  -keystore /path/to/cape-noor-release-key.jks \
  -alias cape-noor-key | grep -E "SHA1|Alias"
```

If Play says expected fingerprint is:

- `F5:1A:9F:EA:C8:3F:CE:2B:B9:8E:E7:78:9D:8F:80:A0:FC:62:B4:9B`

then your local command must print the same SHA1 before you build/upload.

If it prints a different SHA1, you are using the wrong keystore file and must switch `storeFile` in `android/key.properties` to the original upload key.
